# Forge output: ThredUp
_Target: James Reinhart, CEO & Co-founder (or VP Product). Segment: Enterprise. Platform: Proprietary stack. Engine: offline-deterministic. Source: cached whitepaper atoms. Category lift used: 34%._

## Blog post

### The next point of conversion is not on your product page. It is before it.

Most apparel teams pour their attention into the product page and the checkout. The page almost nobody optimizes is the one in between: the moment a shopper lands, scans, and silently decides whether anything here is for them. For ThredUp, every SKU in resale is one of a kind, so discovery is the core battleground, and you already build personalization in-house.
...
Picture the homepage reading the room for ThredUp: a shopper lands, the feed opens on what they came for, then reorders itself with every swipe, surfacing the next best product and the answer to the question they have not typed yet. Every SKU in resale is one of a kind, so discovery is the core battleground, and you already build personalization in-house is, not coincidentally, exactly the kind of problem that gets cheaper when the page learns in real time instead of in quarters. The product page and the checkout are mostly solved. The expensive, unoptimized middle is where the next 34 percent is sitting.
...
Fair instinct, and for a team like ThredUp's, a real option. So here is the honest version.
...
Every SKU in resale is one of a kind, so discovery is the core battleground, and you already build personalization in-house.
...
For ThredUp, discovery is the moat. Most teams just have not pointed their best technology at it yet.
...
ThredUp has put real engineering into AI discovery, which is exactly why the next conversation is build-versus-buy, not whether discovery matters.
...
Worth fifteen minutes to show you what it would look like on a ThredUp page?
...
If now is not the moment, no problem, and I will stop landing in your inbox. If you want to see it on a real ThredUp page first, reply with a time and I will bring the mockup.
