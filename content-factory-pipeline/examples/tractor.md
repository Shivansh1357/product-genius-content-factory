# Forge output: Tractor Supply
_Target: Rick Lockton, SVP E-commerce / Hal Lawton, CEO. Segment: Enterprise. Platform: Custom (Azure). Engine: offline-deterministic. Source: cached whitepaper atoms. Category lift used: 25%._

## Blog post

### The next point of conversion is not on your product page. It is before it.

Most specialty retail teams pour their attention into the product page and the checkout. The page almost nobody optimizes is the one in between: the moment a shopper lands, scans, and silently decides whether anything here is for them. For Tractor Supply, most online orders are buy-online-pickup-in-store, so lifting basket size and online conversion in an omnichannel flow is the stated focus.

We have a number for how expensive that moment is. In controlled A/B tests, adapting to each shopper in real time delivered this: a 36 percent average lift in revenue per session across 25 brands and millions of sessions. In specialty retail specifically, the lift averaged 25 percent. That is the same traffic and the same catalog, converting more of the people already on the page.
...
Picture the homepage reading the room for Tractor Supply: a shopper lands, the feed opens on what they came for, then reorders itself with every swipe, surfacing the next best product and the answer to the question they have not typed yet. Most online orders are buy-online-pickup-in-store, so lifting basket size and online conversion in an omnichannel flow is the stated focus is, not coincidentally, exactly the kind of problem that gets cheaper when the page learns in real time instead of in quarters. The product page and the checkout are mostly solved. The expensive, unoptimized middle is where the next 25 percent is sitting.
...
Fair instinct, and for a team like Tractor Supply's, a real option. So here is the honest version.
...
Most online orders are buy-online-pickup-in-store, so lifting basket size and online conversion in an omnichannel flow is the stated focus.
...
For Tractor Supply, discovery is the moat. Most teams just have not pointed their best technology at it yet.
...
Tractor Supply is already public about leaning into AI for ecommerce. The question is which AI moves a number, and revenue per session is the cleanest one.

We built an AI that turns the few seconds after a shopper lands into a feed that adapts as they scroll, surfacing the next best product instead of making them dig. In controlled A/B tests, the headline result was this: a 36 percent average lift in revenue per session across 25 brands and millions of sessions. In specialty retail specifically, the lift averaged 25 percent.
...
Worth fifteen minutes to show you what it would look like on a Tractor Supply page?
...
If now is not the moment, no problem, and I will stop landing in your inbox. If you want to see it on a real Tractor Supply page first, reply with a time and I will bring the mockup.
